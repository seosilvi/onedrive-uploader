...
async function createFolder(parentId, folderName) {
  ...
  try {
    ...
    if (response.status !== 201) {
      console.error("[ERROR] Failed to create folder. Response status:", response.status);
      console.error("[ERROR] Full response:", data); // Log entire response to capture detailed error information
    }
