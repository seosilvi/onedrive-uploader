...
app.post("/upload", upload.single("file"), async (req, res) => {
    ...
    try {
        const parentFolderId = await getServiceFolderId(form_name.trim());
        if (!parentFolderId) {
...
async function getServiceFolderId(serviceName) {
    const url = `https://graph.microsoft.com/v1.0/drives/b!ROF1vL6LRkekEkmZquo_YIenBUD16f9JkKlgmsFkdBWO-JmMW0_JQYM9CCMhEgyL/items/01HRAU3CM3FJM76AR56FCJ22MAAITZMZOP/children?$filter=name eq '${serviceName}'`;
    ...
    try {
        ...
        if (Array.isArray(data.value) && data.value.length > 0) {
            const folder = data.value[0]; // First matching folder
            return folder.id;
        } else {
            console.error(`Service folder "${serviceName}" not found.`);
            return null;
        }
    } catch (error) {
        console.error("Error fetching service folder ID:", error);
        throw error;
    }
}
